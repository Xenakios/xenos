...
//==============================================================================
XenosAudioProcessorEditor::XenosAudioProcessorEditor(XenosAudioProcessor &p,
      ...
      pitchVisualizer(p.xenosAudioSource.xenosSynth,p.xenosAudioSource.sharedquantizer)
{
    ...
    initParamSlider(mainhpfilter, "mainhpfilterfrequency", "Main highpass filter", horizontal,
                    blue);
    mainhpfilter.setVisible(false);

    if (!audioProcessor.customScaleText.isEmpty())
    {
        scale.changeItemText(customScaleMenuIndex, audioProcessor.customScaleName);
        scale.setItemEnabled(customScaleMenuIndex, true);
        if (dynamic_cast<juce::AudioParameterChoice *>(valueTreeState.getParameter("scale"))
                ->getIndex() == customScaleParamIndex)
        {
...
void XenosAudioProcessorEditor::resized()
{
    ...
    pitchBarrier.setBounds(margin + sliderStep, vSliderY, panel1W / 4, vSliderH);
    pitchStep.setBounds(margin + 2 * sliderStep, vSliderY, panel1W / 4, vSliderH);
    ...
    ampBarrier.setBounds(panel1X2 + sliderStep, vSliderY, panel1W / 4, vSliderH);
    ampStep.setBounds(panel1X2 + 2 * sliderStep, vSliderY, panel1W / 4, vSliderH);
    ...
    sustain.setBounds(panel1X3 + 2 * panel1W / 4, vSliderY, panel1W / 4, vSliderH);
    release.setBounds(panel1X3 + 3 * panel1W / 4, vSliderY, panel1W / 4, vSliderH);
    ...
    pitchWalk.setBounds(margin + panel1W / 2 + margin / 2, panel2Y, menuW, menuH);
    ...
    ampWalk.setBounds(panel1X2 + panel1W / 2 + margin / 2, panel2Y, menuW, menuH);
    ...
    customButton.setBounds(panel1X3 + panel1W * 0.75, panel2Y, panel1W * 0.25, menuH);
...
void XenosAudioProcessorEditor::formatHeaderLabel(juce::Label &label, std::string t)
{
...
void XenosAudioProcessorEditor::initParamSlider(ParamSlider &slider, std::string p, std::string d,
                                                sliderStyle s, juce::Colour c)
{
    slider.param = p;
    slider.disp = d;

    slider.attachment.reset(new SliderAttachment(valueTreeState, slider.param, slider));
    slider.listener = new ParamListener(audioProcessor);
    valueTreeState.addParameterListener(slider.param, slider.listener);
    slider.setTextValueSuffix(" " + valueTreeState.getParameter(slider.param)->getLabel());
    ...
    case vertical:
        ...
        slider.label.attachToComponent(dynamic_cast<juce::Component *>(&slider), false);
        ...
        slider.label.setCentrePosition(slider.getX() + slider.getWidth() / 2,
                                       slider.label.getY() + slider.label.getHeight() * 0.75f);
        break;
    case horizontal:
...
void XenosAudioProcessorEditor::initParamMenu(ParamMenu &menu, std::string p, std::string d,
                                              float labelOffsetX, int numEntriesToAdd)
{
    menu.param = p;
    menu.disp = d;

    auto paramObject =
        dynamic_cast<juce::AudioParameterChoice *>(valueTreeState.getParameter(menu.param));
    ...
    menu.attachment.reset(new ComboBoxAttachment(valueTreeState, menu.param, menu));
...
{
    juce::FileChooser chooser("Select a Scala tuning file (.scl) to load.", juce::File{}, "*.scl");
    if (chooser.browseForFileToOpen())
    {
        ...
        */
        ...
        if (success)
        {
            ...
            audioProcessor.customScaleName = scaleName;
            audioProcessor.customScaleText =
                audioProcessor.xenosAudioSource.sharedquantizer.tuning.scale.rawText;
            // audioProcessor.customScaleData = scaleData;
